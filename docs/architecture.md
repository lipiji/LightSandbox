...
- **Metadata Persistence** (`crates/lightsandbox-runtime-local/src/persistence.rs`, opt-in): an optional write-through `redb` store that mirrors sandbox metadata (id, status, ttl, env, metadata) to a single file so it can be restored after a restart. The in-memory `DashMap` remains the source of truth while the server runs; the store is consulted only once, at startup. Disabled by default (`[persistence] enabled = false`) so the zero-database guarantee from v0.1 is unchanged unless an operator opts in. Workspace *file contents* are already on disk and need no mirroring — only the metadata mapping ids → directories is persisted.
